                ...
                if ($request->isMethod('post')) {
                    ...
                    if ($Validator->fails()) {
                        ...
                        if ($emailExists > 0) // if this is a valid email
                        {
                            ...
                            {
                                return Redirect::Route('admin.'.\App::getLocale().'.forgot.password')->with('error', 'An error occurred while sending you the email containing the password');

                            } else // if password could not be saved successfully
                            {
                                return Redirect::Route('admin.'.\App::getLocale().'.forgot.password')->with('success', 'Password Recovery Link has been sent to your email.');
                            }
                        } else // if this email is not registered
                        {
                            return Redirect::Route('admin.'.\App::getLocale().'.forgot.password')->with('error', 'This email id is not registered');
                        }
                    }
                }
            } catch (Exception $e) {
                return Redirect::Route('admin.'.\App::getLocale().'.forgot.password')->with('error', $e->getMessage());
            }
        }
        ...
        if (Auth::guard('admin')->check()) {
            ...
            try
            {
                if ($request->isMethod('post')) {
                    ...
                    if ($Validator->fails()) {
                        // If validation error occurs, load the error listing
                        return Redirect::Route('admin.'.\App::getLocale().'.reset.password', ['encryptCode' => $encryptString])->withErrors($Validator);
                    } else {
                        ...
                        if (!empty($user)) {
                            ...
                            return Redirect::Route('admin.'.\App::getLocale().'.login')->with('success', 'Your new password successfully updated.');
                        } else // if user not found
                        {
                            return Redirect::Route('admin.'.\App::getLocale().'.login')->with('error', 'Something went wrong.Please try again later.');
                        }
                    }
                }
            } catch (Exception $e) {
                return Redirect::Route('admin.'.\App::getLocale().'.reset.password', ['encryptCode' => $encryptString])->with('error', $e->getMessage());
            }
